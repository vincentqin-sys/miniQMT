            ...
            if hasattr(config, 'ENABLE_SIMULATION_MODE') and config.ENABLE_SIMULATION_MODE:
                ...
                if not positions_df.empty:
                    ...
                    if 'profit_triggered' in positions_df.columns:
                        positions_df['profit_triggered'] = pd.to_numeric(positions_df['profit_triggered'], errors='coerce').fillna(0).astype(bool)
                    if 'profit_breakout_triggered' in positions_df.columns:
                        positions_df['profit_breakout_triggered'] = pd.to_numeric(positions_df['profit_breakout_triggered'], errors='coerce').fillna(0).astype(bool)
                ...
                try:
                    ...
                    if not new_cache.empty:
                        ...
                        if 'profit_triggered' in new_cache.columns:
                            new_cache['profit_triggered'] = pd.to_numeric(new_cache['profit_triggered'], errors='coerce').fillna(0).astype(bool)

                        if 'profit_breakout_triggered' in new_cache.columns:
                            new_cache['profit_breakout_triggered'] = pd.to_numeric(new_cache['profit_breakout_triggered'], errors='coerce').fillna(0).astype(bool)
